...
class GrafikController extends Controller
{
    public function index()
    {
        ...
        $data = DB::table('waqms_valid')
            ->select(
                DB::raw('DATE_FORMAT(created_at, "%Y-%m-%d %H:%i:00") as created_at'),
            ...
            ->groupBy(DB::raw('DATE_FORMAT(created_at, "%Y-%m-%d %H:%i:00")'))
            ->orderBy(DB::raw('DATE_FORMAT(created_at, "%Y-%m-%d %H:%i:00")'))
            ->get();
